...
export const parseExcelToExportJson = async (
  ...
  const Collection = {
    [collectionId]: {
      ...collectionTemplate,
      label: `Excel Import - ${new Date().toLocaleDateString()}`,
  ...
  samples.forEach((smp) => {
    ...
    Sample[smpUuid] = {
      ...
      target_amount_value: smp.target_amount_value || null,
  ...
  samples.forEach((smp) => {
    ...
    const linkData = {
      ...
      sample_id: sampleId,
      equivalent: smp.equivalent || null,
      coefficient: smp.coefficient || 1.0,
    }
